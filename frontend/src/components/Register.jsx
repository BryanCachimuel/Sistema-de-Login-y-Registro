...
const Register = () => {
  ...
  const { nombre, correo, contrasenia } = inputs;
  ...
  const SubmitForm = async (e) => {
    ...
    if(nombre !== "" && correo !== "" && contrasenia !== "") {
      ...
      setLoading(true)
      await axios.post("http://localhost:3005/registro/", Usuario)
      .then(({data}) => {
        setMensaje(data.mensaje);
        setInputs({correo:"",nombre:"",contrasenia:""});
        setTimeout(() => {
          setMensaje("");
          navigate("/login")
          setLoading(false)
  ...
  return (
    <>
      ...
      <div className={styles.formContainer}>
        <h3>Bienvenido a la Página de Registro</h3>
        <form onSubmit={(e) => SubmitForm(e)}>
          ...
          <button type="submit">{loading ? 'Cargando...' : "Registrarse"}</button>
          <p>
